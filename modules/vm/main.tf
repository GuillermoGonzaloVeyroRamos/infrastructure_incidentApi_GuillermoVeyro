...
resource "azurerm_network_security_group" "IN_GROUP" {
...
resource "azurerm_subnet_network_security_group_association" "IN-SGA" {
  ...
  network_security_group_id = azurerm_network_security_group.IN_GROUP.id
}
...
resource "azurerm_linux_virtual_machine" "IN-VM" {
  name                  = "${var.server_name}-${var.enviroment}"
  resource_group_name   = azurerm_resource_group.IN_RG.name
  location              = var.location
  size                  = "Standard_B2s"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo su -c 'mkdir -p /volumes/ngnix/vhostd'",
